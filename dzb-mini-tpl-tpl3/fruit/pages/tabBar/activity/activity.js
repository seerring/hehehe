...
Page({
  ...
  getParams() {
    ...
    if (wx.getStorageSync("orderIndex")) {
      that.setData({
        index01: parseInt(wx.getStorageSync("orderIndex"))
      })
      var a = {
        currentTarget: {
          dataset: {
      ...
      wx.request({
        ...
        success: function(res) {
          ...
          if (res.code == 1) {
            ...
            that.getGroupTime();
            // // 将列表分成两部分,方便页面分两列渲染
            // var list2_1 = that.data.list2.slice(0, Math.ceil(that.data.list2.length / 2));
            // var list2_2 = that.data.list2.slice(Math.ceil(that.data.list2.length / 2), that.data.list2.length);
            // that.setData({
            //   list2_1: list2_1,
            //   list2_2: list2_2
            // })
  ...
  showAddCartWarn() {
    ...
    if (!app.isLogin()) {                 //检测是否登录
      ...
      wx.navigateTo({
        url: '../../mySubpackage/pages/wallet/wallet',
      })
    }
