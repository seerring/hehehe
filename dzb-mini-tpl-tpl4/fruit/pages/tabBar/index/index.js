...
Page({
  ...
  // 跳转拼团
  lookpt() {
    wx.setStorageSync('orderIndex', 1)
    wx.reLaunch({
        ...
        if (res.data.code == 1) {
          ...
          for (let i = 0; i < s.length; i++) {
            if (s[i].id == that.data.clickId) {
              wx.setStorageSync('distance', s[i].km);
            }
          };
          that.setData({
    ...
    wx.request({
      ...
      success: function(res) {
        ...
        if (res.code == 1) {
          that.setData({
            BambooList: res.data,
            wLenth: (res.data.length * 220) + ((res.data.length - 1) * 20) + 'rpx',
          })
        }
      }
    })
      ...
      success: function(res) {
        ...
        if (res.data.length > 0) {
          that.setData({
            ...
            tab_list: res.data,
            tabId: res.data[0].id,
            wLenth2: (res.data.length * 130) + ((res.data.length - 1) * 30) + 60 + 'rpx',
          })
        } else {
          wx.showToast({
            title: '内容显示不完整',
