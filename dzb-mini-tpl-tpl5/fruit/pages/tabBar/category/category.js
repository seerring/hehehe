...
Page({
  ...
  data: {
    ...
    paged: 1, //当前右侧商品页数
    banner: '', //右侧商品banner图
    select: false, //下拉菜单栏是否选中的flag
    selectOption: '优惠推荐', //下拉菜单栏显示的字
  ...
  //搜索后查看相应类型
  searchTabFun: function (e) {
    var that = this;
    var index = e.currentTarget.dataset.index;
    console.log(index)
    var tabList = that.data.searchTabList;
    // that.setData({
    //   searchTabList[index].sort = !tabList[index].sort
    // })
    tabList[index].sort = !tabList[index].sort;
    var so = tabList[index].sort;
    var type = tabList[index].id;
    var cid = that.data.curIndex;
    console.log(that.data.searchTabList)
    // var value = that.data.inputVal;
    that.data.test = !that.data.test
    that.setData({
      curIndexSort: index,
      paged: 1,
      loadingHidden: false,
    ...
    // 点击搜索后的搜索列表
    that.clickSearch(index,cid, so, type);
  },
  // 点击搜索后的搜索列表
  clickSearch: function (index,cid,sort, by) {
    var that = this;
    console.log(that.data.curIndexSort)
    console.log(index)
    console.log(that.data.searchTabList[index].sort)
  ...
  tabFun(e) {
    ...
    var index = e.currentTarget.dataset.index;
    var name = e.currentTarget.dataset.name; //下拉框要选中显示的名字
    that.setData({
      curIndex: curIndex,
      paged: 1,
      tabId: index,
